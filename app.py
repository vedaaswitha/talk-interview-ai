...
print(f"[DEBUG] AssemblyAI Key loaded: {bool(ASSEMBLYAI_API_KEY)}")  # add this
...
def stream_audio(text):
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"[Murf API] Error body: {response.text}")
            return
